...
async function start() {
  ...
  db.run(`CREATE TABLE IF NOT EXISTS usage_log (id INTEGER PRIMARY KEY AUTOINCREMENT, material_type TEXT NOT NULL CHECK(material_type IN ('filament','resin','paint')), material_id INTEGER NOT NULL, amount_used REAL NOT NULL, unit TEXT NOT NULL CHECK(unit IN ('g','ml')), project_name TEXT, notes TEXT, used_at TEXT DEFAULT (datetime('now')))`);
  db.run(`CREATE TABLE IF NOT EXISTS purchase_history (id INTEGER PRIMARY KEY AUTOINCREMENT, material_type TEXT NOT NULL CHECK(material_type IN ('filament','resin','paint')), material_id INTEGER NOT NULL, quantity INTEGER DEFAULT 1, unit_price REAL NOT NULL, total_price REAL NOT NULL, currency TEXT DEFAULT 'PEN', supplier TEXT, purchased_at TEXT DEFAULT (datetime('now')), notes TEXT)`);
  ...
  app.post('/api/filaments', (req, res) => {
    ...
    runSql('INSERT INTO purchase_history (material_type,material_id,unit_price,total_price,currency,supplier,purchased_at) VALUES (?,?,?,?,?,?,?)',
      ['filament', result.lastInsertRowid, price, price, currency||'PEN', supplier, purchase_date||new Date().toISOString()]);
  ...
  app.post('/api/resins', (req, res) => {
    ...
    runSql('INSERT INTO purchase_history (material_type,material_id,unit_price,total_price,currency,supplier,purchased_at) VALUES (?,?,?,?,?,?,?)',
      ['resin', result.lastInsertRowid, price, price, currency||'PEN', supplier, purchase_date||new Date().toISOString()]);
  ...
  app.post('/api/paints', (req, res) => {
    ...
    runSql('INSERT INTO purchase_history (material_type,material_id,unit_price,total_price,currency,supplier,purchased_at) VALUES (?,?,?,?,?,?,?)',
      ['paint', result.lastInsertRowid, price, price, currency||'PEN', supplier, purchase_date||new Date().toISOString()]);
